...
import { getStaffProfileByMemberId } from "../getStaffProfileByMemberId";
...
describe("getStaffProfileByMemberId", () => {
  const headers = createMockHeaders();

  it("should return staff profile when member has a staff profile", async () => {
    ...
    const result = await getStaffProfileByMemberId({
      container,
      headers,
      input: { memberId },
    });
  ...
  it("should return staff profile with assigned menus", async () => {
    ...
    await assignMenus(container.db, staffProfileId, [menuId1, menuId2]);
    ...
    expect(result.assignedMenus).toHaveLength(2);
  ...
  it("should throw NotFoundError when member has no staff profile", async () => {
    const container = getContainer();
    const tenantId = await insertTenant(container.db);
    const memberId = await insertMember(container.db, tenantId, {
      role: "admin",
  ...
  it("should throw NotFoundError when memberId does not exist", async () => {
    const container = getContainer();
    ...
    await expect(
      getStaffProfileByMemberId({
        container,
        headers,
        input: { memberId: nonExistentMemberId },
      }),
    ).rejects.toThrow(NotFoundError);
  });
});
